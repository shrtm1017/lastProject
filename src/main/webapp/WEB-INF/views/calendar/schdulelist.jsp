...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#scheduleInsert").on("click", function() {
			...
			$("#scd_str_dt").val(startStr);
			$("#scd_end_dt").val(endStr);
			...
			else if ($("#scd_str_dt").val() > $("#scd_end_dt").val()) {
		...
		$("#updateSchedule").on("click", function() {
			var params = {
				bs_scd_div_sq : $("#bs_scd_div_sq_up").val(),
	...
	function selectSchedule() {
		...
		$.ajax({
			...
			success : function(data) { // 여기 data는 selectSchedule에서 넘어온 data(result)
				...
				$.each(data, function(idx, val) { // $.each : 반복문   data 매개변수, idx 인덱스, val 키값
					// num : 몇일인지를 나타내는 변수
					...
					var startDay = new Date(val.scd_str_dt.substring(0, 10));
					var endDay = new Date(val.scd_end_dt.substring(0, 10));
					...
					for (var i=0; i<subDay; i++) {
						...
						if (arr[num + i] < 3) {
							arr[num + i]++;
							str = "<div class='scheduleDiv' data-toggle='tooltip' data-placement='top' title='" + val.scd_title + "' data-value='" + val.scd_sq + "' data-toggle='modal' data-target='#myModal'>" + (val.scd_title.length<8?val.scd_title:(val.scd_title.substring(0, 8)+" ...")) + "</div>";
	...
	function readSchedule(params) {		
		$.ajax({
			...
			success: function(data) {			    
			    $("#bs_scd_div_sq_up").val(data.scd_div_sq);
			    $("#bs_scd_nm_up").val(data.scd_title);
			    $("#stf_nm_up").val(data.scd_emp_sq);
			    $("#bs_scd_str_dt_up").val(data.scd_str_dt);
			    $("#bs_scd_end_dt_up").val(data.scd_end_dt);
			    $("#bs_scd_cnt_up").text(data.scd_con);
			},
			error: function(request, status, error) {
	...
	function updateSchedule(params) {		
		$.ajax({
			...
			success: function(data) {	    
				if (data == 1) {
					window.location = "/scheduleList?scd_div_sq=" + $("#scd_div_sq").val();
				}
				else if (data != 1) {
					alert("수정을 실패하였습니다.");
				}
	...
	function deleteSchedule() {
		...
		$.ajax({
			...
			success: function(data) {			     
			    if (Number(data) == 1) {
			    	window.location = "/scheduleList?scd_sq=" + $("#scd_sq").val();
			    } 
	...
	<div id="wrap">
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form>
							<c:choose>
								<c:when test="${!empty scd_emp_sq || scd_div_sq eq 1}">
									<div class="form-group">
										<input type="hidden"
											class="form-control" id="bs_scd_div_sq_up" name="bs_scd_div_sq"
											readonly="readonly" value="${scd_sq }">
									</div>
									<div class="form-group">
								...
								<c:otherwise>
									<div class="form-group">
										<input type="hidden"
											class="form-control" id="bs_scd_div_sq_up" name="bs_scd_div_sq"
											readonly="readonly" value="${scd_div_sq }">
									</div>
									<div class="form-group">
										<label for="title">일정명</label> <input type="text"
											class="form-control" id="bs_scd_nm_up" name="bs_scd_nm"
		...
		<div id="content">
			<div id="title">
				<h3>
					<c:choose>
						<c:when test="${scd_div_sq == 1}">개인 일정</c:when>
						<c:when test="${scd_div_sq == 2}">부서 일정</c:when>
						<c:when test="${scd_div_sq == 3}">회사 일정</c:when>
						<c:when test="${scd_div_sq == 4}">업무 일정</c:when>
					</c:choose>
				</h3>
			</div>
			...
			<div>
				...
				<div id="leftMenu" class="col-md-3">
					...
					<div id="leftBottom">
						<form id="scheduleInsertForm" action="${cp }/insertSchedule">
							<input type="hidden" id="scd_div_sq" name="scd_div_sq" value="${scd_div_sq }">
							<div class="form-group">
								<label for="scheduleTitle1">제목</label> 
								<input type="text" class="form-control" id="scd_title" name="scd_title" placeholder="제목">
							</div>
							...
							<div class="form-group">
								...
								<input type="hidden" id="scd_str_dt" name="scd_str_dt" />
							</div>
							...
							<div class="form-group">
								...
								<input type="hidden" id="scd_end_dt" name="scd_end_dt" />
							</div>
							
							<div class="form-group">
								<label for="scheduleContent1">내용</label> 
								<textarea id="scd_con" name="scd_con" class="form-control" rows="7"></textarea>
							</div>
