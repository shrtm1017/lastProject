	...
	<div class="col-sm-4">
	    <div class="page-header float-left" id="page-title-style">
	        <div class="page-title">
	            <h1 style="padding-bottom: 0px;"><strong>내가 올린 결재</strong></h1>
	        </div>
	    </div>
	</div>
</div>
...
<div>
	<div class="col-lg-12 col-sm-4"> <br>
		...
		<form id="frm"  action="${cp }/apvSendSearch" method="get" class="form-horizontal" role="form">
				...
				<c:choose>
					...
					<c:otherwise>
						<li id="bootstrap-data-table_previous">
							<a href="${cp}/apvSendSearch?page=1&selDt=${selDt}&apv_str_dt=${apv_str_dt}&apv_end_dt=${apv_end_dt}&apv_state=${apv_state}&apv_div_sq=${apv_div_sq}&apv_nm=${apv_nm}" aria-controls="bootstrap-data-table" tabindex="0" class="page-link"><i class="fa fa-angle-double-left"></i></a>
						</li>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					<c:when test="${startPage == lastPageFirstValue}">
						<c:forEach begin="${lastPageFirstValue}" end="${lastPage}" var="i">
							...
							<li class="${active}">
								<a href="${cp}/apvSendSearch?page=${i}&selDt=${selDt}&apv_str_dt=${apv_str_dt}&apv_end_dt=${apv_end_dt}&apv_state=${apv_state}&apv_div_sq=${apv_div_sq}&apv_nm=${apv_nm}" aria-controls="bootstrap-data-table" data-dt-idx="1" tabindex="0" class="page-link">${i}</a>
							</li>
						</c:forEach>
					</c:when>
					<c:otherwise>
				...
				<c:choose>
					...
					<c:otherwise>
						<li id="bootstrap-data-table_next">
							<a href="${cp}/apvSendSearch?page=${lastPage}&selDt=${selDt}&apv_str_dt=${apv_str_dt}&apv_end_dt=${apv_end_dt}&apv_state=${apv_state}&apv_div_sq=${apv_div_sq}&apv_nm=${apv_nm}" aria-controls="bootstrap-data-table" tabindex="0" class="page-link"><i class="fa fa-angle-double-right"></i></a>
						</li>
					</c:otherwise>
				</c:choose>
			</ul>
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" style="display: none;" id="resubmitBtn" class="btn btn-secondary btn-sm col-sm-2">재상신</button>
				&nbsp;
...
<input type="hidden" id="send_apv_sq" />
	...
	jquery("#detailModal").on("show.bs.modal", function(event){
		...
		jquery.ajax({
			...
			success  : function(data) {
				...
				// 반려 문서 - 재상신 기능
				if (data.apv_state == '4') {
					jquery("#resubmitBtn").show();
				} else {
					jquery("#resubmitBtn").hide();
				}
